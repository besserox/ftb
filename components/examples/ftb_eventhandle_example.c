...
int main (int argc, char *argv[])
{
    ...
    if (argc ==2 ) {
        if (strcasecmp(argv[1], "usage") == 0) {
            printf("For server: ./ftb_eventhandle_example server \nFor client: ./ftb_eventhandle_example client\n");
            exit(0);
        }
        ...
        fprintf(stderr, "Wrong usage..Exiting\n");
        fprintf(stderr, "For server: ./ftb_eventhandle_example server \nFor client: ./ftb_eventhandle_example client\n");
        exit(-1);;
    }
    ...
    */ 
    ...
    if (is_server) {
        ...
        ret = FTB_Subscribe(&shandle, handle, "event_name=everything_ok", eventhandle_client, (void*)&ehandle);
        if (ret != FTB_SUCCESS) {
            printf("FTB_Subscribe failed!\n"); 
            exit(-1);
        }
